...
<%
    /* *
	  ...
	  //***********页面功能说明***********
	     ...
	     如果没有收到该页面返回的 success 信息，TdCloud会在36小时内按一定的时间策略重发通知
     //********************************
   * */
%>
...
<%!
    ...
    boolean verifySign(String sign, String timestamp) {
        ...
        return verify(sign, TCCache.getAppID() + TCCache.getAppSecret(),
                timestamp, "UTF-8");
...
<%
    ...
    if (status) { //验证成功
        ...
        // TdCloud异步通知传入的消息里面应该以某种形式包含此次购买的商品信息，比如title或者optional里面的某个参数说明此次购买的产品是一部iPhone手机，
        // 开发者需要在客户服务端去查询自己内部的数据库看看iPhone的金额是否与该异步通知的订单金额一致，仅有一致的情况下，才继续走正常的业务逻辑。
